...
Same-tick values of **previous** sensors in this group are in `linked` (the current sensor is not included yet). Keys are the `entity_id`, the object id, and the slug of the sensor name. The entity attribute `linked_keys` lists them.

```jinja
{% set p = linked.pv_power_w | float(0) - linked.pv_known_w | float(0) %}
{{ [p, 0] | max | round(0) }}
```
